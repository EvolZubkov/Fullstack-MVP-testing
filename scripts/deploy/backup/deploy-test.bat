...
:: Non-secret config: the image carries none (config/ is a host volume), so the
:: repo's config\test.config.jsonc is shipped and seeded on the server.
set "REMOTE_CONFIG=/tmp/deploy-test-%PROJECT_NAME%.test.config.jsonc"
set "LOCAL_CONFIG=%SCRIPT_DIR%..\..\config\test.config.jsonc"
...
echo   Shipping config\test.config.jsonc ^(non-secret config^)...
scp "%LOCAL_CONFIG%" "%DEPLOY_TARGET%:%REMOTE_CONFIG%"
