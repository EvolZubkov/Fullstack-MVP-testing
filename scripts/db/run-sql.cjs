 ...
 *
 * Usage: node scripts/db/run-sql.cjs <path-to-sql-file> [<more-files> ...]
 */
...
async function main() {
  ...
  if (files.length === 0) {
    console.error("[run-sql] usage: node scripts/db/run-sql.cjs <file.sql> [<file.sql> ...]");
    process.exit(1);
  }
