...
ssh -tt "%DEPLOY_TARGET%" "docker load -i %REMOTE_TAR% && sudo sed -i 's|image: .*:latest|image: %IMAGE_NAME%:latest|' %TEST_APP_DIR%/docker-compose.yml && cd %TEST_APP_DIR% && sudo docker compose up -d --force-recreate && echo 'Waiting for container...' && sleep 5 && echo 'Applying pre-push data migrations (016 roles + PRD-15 023/024/026/027/028)...' && sudo docker exec %IMAGE_NAME% node script/run-sql.cjs migrations/016_prd13_rbac_roles.sql migrations/023_td02_topic_feedback_json.sql migrations/024_td02_drop_topic_courses_events.sql migrations/026_prd15_test_question_scoring.sql migrations/027_prd15_scoring_backfill.sql migrations/028_prd15_drop_question_scoring_columns.sql && echo 'Verifying schema before push (PRD-15 gate)...' && sudo docker exec %IMAGE_NAME% node script/run-sql.cjs script/verify-prd15-pre-push.sql && echo 'Applying DB schema (drizzle-kit push)...' && sudo docker exec -it %IMAGE_NAME% npx drizzle-kit push --force && rm -f %REMOTE_TAR%"
