...
set "DEPLOY_ENV=%DOCKER_DIR%\config\deploy.env"
...
:: Required inputs
:: ---------------------------------------------------------------------------
if not exist "docker\scripts\deploy.sh"            ( echo ERROR: docker\scripts\deploy.sh not found            & exit /b 1 )
if not exist "docker\scripts\run-deploy.sh"        ( echo ERROR: docker\scripts\run-deploy.sh not found        & exit /b 1 )
...
echo     docker\scripts\deploy-prod.bat ^<user@server^> [--no-build]
echo     docker\scripts\deploy-test.bat ^<user@server^> [--reset-db] [--no-build]
echo.
echo Example:
echo   docker\scripts\deploy.bat vvlad1973@192.168.1.200 test
exit /b 1
