...
| --- | --- | --- | --- |
| 2026-05-17 | 3.3b | `radio-row`, `radio-opt`, `toggle`, `toggle-row` → переписать на DS `ou-switch` (track + thumb + is-on) и `ou-radio` (ring + dot). Структурный refactor. | Открыт. |
| 2026-05-17 | 3.3c | Голые `<input>`, `<select>`, `<textarea>` нужно обернуть в DS BEM: `ou-field` + `ou-field__box` + `ou-field__input`; `ou-textarea` + `__box` + `__input`; `ou-select` с custom trigger/menu (статика). Затронет сотни мест в 4 файлах. До этого момента прямые селекторы `input[type=...]`, `select`, `textarea` оставлены в `prd7-shared.css` как WIREFRAME-INFRA-glue. | Открыт. |
| 2026-05-17 | 3.3c-blocker | `ou-select` в DS — JS-компонент (`__trigger`/`__menu`/`__opt`). В wireframe можно показать только trigger-состояние. Для wireframe-needs допускается статический snapshot без меню. | Решено: статический snapshot. |
