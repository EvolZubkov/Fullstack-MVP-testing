...
| 2026-05-17 | 4a-gap | `modal-option`, `modal-option-title`, `modal-option-sub`, `modal-option-icon`, `modal-option-danger`, `modal-inset` — radio-card паттерн внутри модалки (`prd7-tests-list.html`). DS не предоставляет эквивалент; исключены из gate `legacyClassTokens` с пометкой. Кандидат на расширение DS API. | Открыт. |
