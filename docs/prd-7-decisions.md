...
- PRD-4, prd-7-test-settings-editor-refactor.md, test-settings-parameter-structure.md
...
4. При смене `flowMode` обратно на `linear_flat` / `linear_by_topics`:
   router-row **удаляется системой молча** (без диалога `s-mapping`). В
   `s-mode-change` warning-callout вкладки «Структура» в раскрываемом списке
   преобразований фигурирует пункт «Маршрутизатор будет удалён».
5. Темы не удаляются — они зеркалируют состав теста (вкладка «Состав») и
   переходят в общий поток при переключении режима.
