...
| 2026-05-17 | 3.3c-deferred | Wireframe-стилизованные классы `wf-text-input`, `wf-textarea`, `wf-select`, `wf-mob-input`, `wf-num-input`, `wf-tbl-input-*`, `wf-failure-input`, `wf-json-textarea`, `wf-link-input`, `wf-format-select`, `wf-flow-select`, `wf-type-select`, `wf-tbl-select`, `wf-field-input`, `wf-draw-count-input`, `draw-count-input` в 9 файлах. Эти классы прятали продакшн-стили за wf-префиксом. | Закрыт (3.3d). |
| 2026-05-17 | 3.3d | Все wf-*-input / -select / -textarea (плюс `draw-count-input` и `wf-mob-settings-selector`) мигрированы на DS BEM: `<input>/<select>/<textarea>` обёрнуты в `ou-field/ou-field__box` или `ou-textarea/ou-textarea__box` с `ou-field__input` / `ou-textarea__input`; error-варианты на `ou-field--error` / `ou-textarea--error`. Соответствующие имена добавлены в `legacyClassTokens` gate как регрессионный щит. Локальный CSS для этих классов остаётся в `<style>` блоках файлов как мёртвый код (не блокирует gate, удаление — отдельный косметический cleanup). | Закрыт. |
| 2026-05-17 | 4a-gap | `modal-option`, `modal-option-title`, `modal-option-sub`, `modal-option-icon`, `modal-option-danger`, `modal-inset` — radio-card паттерн внутри модалки (`prd7-tests-list.html`). Изначально DS не предоставлял эквивалент. Закрыт через расширение DS: добавлен компонент `ou-choice-card` (ui-kit React-компонент `ChoiceCard` + Storybook history `Form/ChoiceCard`). Wireframe мигрирован: `modal-option*` → `ou-choice-card[/--destructive]/__lead/__body/__title/__desc/__inset`. CSS-правила удалены из локального `<style>` блока. Имена `modal-option-*` / `modal-inset` возвращены в `legacyClassTokens` gate как регрессионный щит. | Закрыт. |
| 2026-05-17 | 4b-c | inline raw colors / direct units: `data-wf-demo` ancestor whitelist (gate сделан стек-парс HTML), token-map `Npx -> var(--ou-space-N)`, локальные `--wf-*` токены в `prd7-shared.css` (внутри `/* gate-skip-start ... gate-skip-end */`), замены named-color `white/black` на `--ou-neutral-0/950`. | Закрыт. |
| 2026-05-17 | 4d | DS не имеет width / hairline / line-height / breakpoint токенов. Локальный слой `--wf-border-w`, `--wf-space-*`, `--wf-size-*` фиксирует литералы 1px / 1.5px / 2px / 3px / 5-15px / 18-959px / 1120-1200px / `--wf-radius-pill: 9999px`. Зона `em` исключена из gate (типографика). | Закрыт. |
