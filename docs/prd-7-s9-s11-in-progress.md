...
**Статус:** In Implementation (S9 active)
...
- **S10** — Удаление legacy (inline wizard в `tests.tsx`, чтение `tests.published` и
...
- [ ] **FR-20c** — реализовать ссылки-якоря из сводки секции к проблемным полям,
  используя `field`-пути из `ValidationIssue` (единственный незакрытый пункт S4/4A).
...
### 2.4 Definition of Done для S9

- Все пункты чек-листов §2.2, §2.3 закрыты тестами.
- FR-20c реализован: anchor-навигация работает + покрыта тестом.
- `npm run check` зелёный.
- `npx vitest run` полный suite зелёный (включая existing tests).
- Нет регрессий по ~457 тестам из S0-S8.
...
| --- | --- |
| S9 | S0-S8 закрыты (готово 2026-05-25) |
| S10 | S9 зелёный (regression подтверждён) |
| S11 | S9 + S10 закрыты; полный набор UI работает в браузере |

**Внешних блокеров нет.** S9 запускается без ожиданий — production-код секций
существует, reference-тесты есть.

**Параллельные потоки внутри S9:** часть 1 (component) и часть 2 (API + regression)
могут идти параллельно — разные кодовые зоны. Рекомендовано выполнять
последовательно через `/model`-переключение в одной сессии для удешевления
cache-warming.
