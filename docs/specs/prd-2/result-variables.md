...
#### 4.2.4 За рамками и планируемое

- За рамками текущей версии: `tag("name").{percent,score,maxScore,count}` (под-темы PRD-11) и
...
- `score` (общий балл теста) — токен `score` в `shared/formula/*` + наполнение в обоих
  рантаймах (`Σ` баллов по темам, scoring-model §11.7).
