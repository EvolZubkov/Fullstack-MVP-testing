...
**Реализовано** (парный трек с PRD-2, Этапы A-C). Таблицы `scales` + `question_measurements`
(миграция 009), движок `shared/scales/engine.ts` + JS-порт (golden-паритет), API
`/api/tests/:id/scales` + `/measurements` + `scales/preview`, рантайм-расчёт `scale.*` в
`resultsPage.js` (псевдо-интеракции `scale_{key}[_level]`, `suspend_data.custom.scale`),
экспорт `test.scales[]`/`test.measurements[]`, вкладка «Шкалы» (список + bands + preview +
матрица «Вклады вопросов»). Сквозной MBI-сценарий — golden-тест `tests/mbi-golden.test.ts`.

Решения по двум исходным пререквизитам: (1) градуированный `checkAnswer` НЕ потребовался —
вклад задаётся ЯВНЫМ `value_json` в сетке «вариант × шкала» (correctness в шкалы не
наследуется, §4.2); (2) стабильные ID единиц ответа отложены — `source_key` индексный
(`option`→индекс, `matching_pair`→`l:r`, `ranking_position`→`item:pos`); гард на
переупорядочивание/удаление опций с измерениями в редакторе вопросов — отложенный пункт
([scoring-model.md](../scoring-model.md) §10.7). Композитные шкалы (источник «Другие
шкалы») и глобальная библиотека шкал — будущий PRD.
