...
**Версия:** 1.0  
**Статус:** Phase 1 реализована (2026-06-04); Phase 2 отложена (post-MVP)  
...
- [ ] Администратор может изменить endpoint, параметры, фильтр статусов, regex прогресса и поле
      даты без изменения Core (Phase 1 — реестр read-only, сидируется в коде).
