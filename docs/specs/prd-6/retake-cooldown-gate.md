...
- [x] Два разных теста могут использовать разные plugins (одна активная конфигурация на plugin в Phase 1).
...
- [x] В blocked-сценарии `cmi.completion_status`, `cmi.success_status`, `cmi.suspend_data` и `cmi.session_time` не записываются (NFR-02).
- [x] WebTutor plugin получает дату прохождения курса через ClientBridge `get_metadata` (подтверждено на живом портале RT) и применяет cooldown.
...
Phase 2 (отложено — администрируемый реестр):

- [ ] Администратор может изменить endpoint, параметры, фильтр статусов, regex прогресса и поле даты без изменения Core (Phase 1 — реестр read-only, сидируется в коде).
- [ ] Выбор одной из ≥2 конфигураций plugin в UI (Phase 1 — одна активная конфигурация подставляется неявно).
