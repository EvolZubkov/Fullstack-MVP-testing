...
шаблона, а не молча пропущенная запись. Проверка выполняется при загрузке И при активации
(PRD-3 NFR-05/06); при загрузке дополнительно сверяется, что у каждой объявленной темы есть
хотя бы один свой токен в стилях.
...
экран, применяется fallback на стандартный шаблон с предупреждением в UI (PRD-1 §4.3.2,
PRD-3 NFR-06) — колонка «Fallback» ниже.

> **PRD-22.** Отдельного вида страницы «галерея» больше нет: галерея — обычный вариант
...
- **Прочие контентные страницы** (`info` / `summary` / `router` / `html`) резолвятся по
  ...
  и отдельного правила не имеет (PRD-22).
...
| `content.info` | Информационная/учебная страница; сюда же относится слайд галереи — вариант со своим `layoutFile` (PRD-22) | `layoutFile` варианта, иначе `layouts["content"]` | Нет: недоступный вариант подменяется вариантом по умолчанию `kind`, замену подтверждает автор (§8.2.2, PRD-1 §4.3.2) |
...
> **PRD-22.** Перечень приведён к фактически поддерживаемому и объявлен однократно в общем коде
...
#### `settings[]` — свойства страницы (PRD-22)
...
### 8.2.2 Недоступный вариант: подмена вариантом по умолчанию (PRD-22)
...
выражениями без DOM. Поэтому макет и его CSS должны выдерживать чужую разметку: изолируйте
оформление классами компонентов и не полагайтесь на глобальные селекторы. Полная таблица правил
и практические выводы — [руководство §8.2](../guides/template-development.md).

> **PRD-22 (согласован, внедряется поэтапно).** Объём допустимого HTML не меняется — он равен
...
>
> **PRD-22.** Контентная страница дополнительно получает неймспейс `page.*`, вычисленный ядром
> из структуры теста:
...
- неизвестный тип поля в `placeholders[]` или `settings[]`, а также несовместимые атрибуты типа
  (например `select` без списка вариантов) — PRD-22;
...
>
> **PRD-22 (согласован, внедряется поэтапно).** Проверка типов полей выполняется при ЗАГРУЗКЕ и
