...
Макет экрана, у которого есть вариант в `contentTemplates[]`, задаётся полем `layoutFile` этого
варианта (§8.2). Ключи `layouts[]` используются только системными экранами без варианта. Когда
шаблон не объявляет нужный экран, применяется fallback на стандартный шаблон с предупреждением в
UI (PRD-1 §4.3.2, PRD-3 NFR-06) — колонка «Fallback» ниже.

| Page kind | Назначение | Откуда макет | Fallback |
| --- | --- | --- | --- |
| `start` | Стартовый экран теста (лендинг) — тест-уровневый, всегда | `layoutFile` варианта `kind: start` | Вариант `start` стандартного шаблона |
| `content.intro` | «Введение раздела» (`before_topic`) — по одной на тему | `layoutFile` варианта `kind: intro` | Вариант `intro` стандартного шаблона |
| `content.info` | Информационная/учебная страница | `layoutFile` варианта `kind: info` | Нет: несовместимость разрешает автор через диалог маппинга (PRD-1 §4.3.2) |
| `content.summary` | «Итог раздела» (`after_topic`) — по одной на тему; показывает результат РАЗДЕЛА | `layoutFile` варианта `kind: summary` | Вариант `summary` стандартного шаблона |
| `content.router` | Меню тем (`router_by_topics`) | `layoutFile` варианта `kind: router` | Вариант `router` стандартного шаблона |
| `question` | Экран вопроса | `layoutFile` варианта `kind: questions` | Вариант `questions` стандартного шаблона |
| `results` | «Итоги теста» — итоговый результат всего теста, тест-уровневый, всегда | `layoutFile` варианта `kind: results` | Вариант `results` стандартного шаблона |
| `review` | «Обзор раздела/теста» перед завершением (PRD-19) — системный узел | `layouts.review` | `layouts.review` стандартного шаблона |
| `section-results` | Вычисляемые «Итоги раздела» (PRD-19) — системный узел | `layouts["section-results"]` | `layouts["section-results"]` стандартного шаблона |
| `content.html` | Санитизированный HTML-блок | `layouts.content` | `layouts.content` стандартного шаблона |
| `system.blocked` | Системная страница блокировки | `systemPages[].layout` / `layouts["system.blocked"]` | `layouts["system.blocked"]` стандартного шаблона |
...
> `"layouts/start.html"`. Ключа `layout` в контракте нет. См. эталонный
...
`content.intro` и `content.summary` — симметричные «закладки» раздела (PRD-4): `content.intro`
