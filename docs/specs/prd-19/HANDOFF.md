...
- Блок F (FR-19/20) — ГОТОВ/ЗАКОММИЧЕН (`75941a6`). Остаток блока F: (1) ВЕБ-паритет cooldown
  (серверный `retake-gate.ts` + ученический старт через тот же шаблон); (2) SCORM eligible-повтор
  (FR-19 «повтор: можно») — prior-результат + «Скачать отчёт» ПОСЛЕ Initialize в `startPage.js` через
  `getBestAttempt` + обработчик `download-report` (контракт `state.priorResult`/`canDownloadReport`
  уже есть, layout/CSS готовы — нужна проводка).
