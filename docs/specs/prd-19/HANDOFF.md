...
## Обновление 2026-06-29 (сессия 4): ВЕБ-паритет cooldown + блок G (FR-24) — РЕАЛИЗОВАНЫ, НЕ закоммичено

Закрыты ОБА остаточных пункта трека. Рабочее дерево грязное (не закоммичено). `tsc` чист.
Полная сюита зелёная: 154 файла, 2829 тестов.

### ВЕБ-паритет cooldown (остаток блока F, FR-19/20) — ГОТОВ

Cooldown теперь рендерится НА стартовой странице веб-хоста через тот же общий шаблон
(`buildStartState` + `start.html`), без отдельной блок-стены — паритет с SCORM-гейтом
`renderCooldownStart`.

- Сервер `GET /api/learner/tests` (`server/routes/attempts.ts`): на каждый тест считает
  `decideRetake` (источник даты — собственные завершённые попытки сервера, без LMS-плагина;
  `retake-gate.ts`) и отдаёт `retakeGate` (`{ cooldownPeriodDays, availableDate }` | null) +
  `priorResult` (`{ percent, passed, attemptNumber, maxAttempts }` из `resultJson` ПОСЛЕДНЕЙ
  завершённой попытки — той же, на которую указывает `lastCompletedAttemptId`/«Мой результат»).
- Клиент `take-test.tsx`: `testMetadata.retakeGate/priorResult`; стартовый `buildStartState`
  получает `cooldown` (дата ДД.ММ.ГГГГ + «через N дн.» через новые чистые хелперы
  `client/src/pages/learner/cooldown-format.ts`, зеркало SCORM `fmtDateHuman`/`daysUntilIso`) и
  `priorResult`. Веб без клиентского PDF → `canDownloadReport` НЕ выставляется (кнопка «Скачать
  отчёт» скрыта; «Мой результат» ведёт на результат). На 403 RETAKE_COOLDOWN (гонка) — cooldown
  сворачивается в `retakeGate` и остаётся на стартовой (старая блок-стена — только fallback при
  отсутствии `startTpl`).
- Тесты: `client/src/pages/learner/__tests__/cooldown-format.test.ts` (формат даты + daysUntil).
  Модель cooldown/priorResult уже покрыта `tests/start-state.test.ts`.
- ВИЗУАЛЬНО ПРОВЕРЕНО через Playwright (скрин `.playwright-mcp/prd19-web-cooldown-start.png`):
  cooldown-карточка НА стартовой («Повторное прохождение пока недоступно» + дата 29.07.2026 +
  «через 30 дн.» + «Прошлая попытка: 60% · не пройдено · попытка 1 из 3» + «Мой результат»),
  кнопка «Начать тестирование» disabled, «Скачать отчёт» отсутствует (нет веб-PDF), отдельной
  блок-стены нет.

### Блок G — статусы пропуска/возврата в отладчике (FR-24) — ГОТОВ

- «Состояние» уже показывает `state.questionStatuses` (дерево всего `state`).
- «Протокол»: добавлено поле `status` ('answered'|'skipped'|'unanswered') в строку протокола.
  Источник — ЖИВАЯ карта `pkg.state.questionStatuses` (отлична от сырого наличия ответа: пропуск
  с черновиком = `answered:true`, но `status:'skipped'`). Прошлые попытки (suspend) — деривация из
  наличия ответа. Изменения: общий `inspector-compute.js buildProtocolRows`; тип `ProtocolRow`
  (`inspector-snapshot.ts`) + колонка «Статус» (`statusTag`) в React-таблице
  (`debug-player-page.tsx`) + столбец в CSV (`protocolToCsv`); CLI `inspector.js` (тег + CSV) —
  паритет, т.к. CLI и веб делят ОДИН compute.
- Тесты: `tests/debug-player-reference.test.ts` (реальный compute — статус из questionStatuses +
  деривация без карты), `tests/debug-player-snapshot.test.ts` (CSV-колонка «Статус»).
- ВИЗУАЛЬНО ПРОВЕРЕНО через Playwright (скрин `.playwright-mcp/prd19-debug-protocol-status.png`):
  отладчик `/author/tests/:id/debug`, Протокол показывает колонку «Статус» — Q1 «отвечен»/вердикт
  «неверно» (статус НЕ зависит от верности), Q2 «пропущен»/«нет ответа»; пиллы в плеере
  (отвечен/пропущен/текущий/не выдан); «Состояние» рендерит весь `state` (questionStatuses внутри,
  путь не менялся).

### Гоча окружения (важно для следующей сессии)

dev-БД `test_builder` (Docker :55432) — СТАРЫЙ снимок схемы (нет `user_roles`, `scales`,
`test_snapshots` и др. PRD-13+; ещё есть дропнутые `topic_courses`/`topic_events`) → текущий код на
ней не запускается (login 500, `/api/tests` падает). Для веб-визуала подняли ИЗОЛИРОВАННУЮ БД
`prd19qa` в том же контейнере (`drizzle-kit push --force` + автосид `admin@test.com`/`admin123`,
`learner@test.com`/`learner123`), отдельный сервер на :8081 с override `DATABASE_URL`; после проверки
...
- Блок F (FR-19/20) — ГОТОВ. SCORM-часть ЗАКОММИЧЕНА (`75941a6` cooldown-старт + снятие гейт-шелла;
  `4075a20` eligible-повтор «повтор: можно»). ВЕБ-паритет cooldown РЕАЛИЗОВАН в сессии 4 (НЕ
  закоммичено) — см. блок «Обновление сессии 4» выше.
- Блок G (статусы пропуска/возврата в отладчике PRD-18, FR-24) — РЕАЛИЗОВАН в сессии 4 (НЕ
  закоммичено) — см. блок «Обновление сессии 4» выше.
