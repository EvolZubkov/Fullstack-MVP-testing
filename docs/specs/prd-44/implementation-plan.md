...
- **Фаза 1 (ядро) — ГОТОВА** 2026-08-06, пять коммитов: тип и признаки, `dataJson` и перечисления схемы,
  чистая модель распределения, вклад в шкалы с доменом и референсом ЧИЛ, паритет JS-двойника.
  Приёмка A-02 закрыта тестом: 34 / 16 / 14 / 34 при сумме 98, домен стиля 0..98.
- Остальные фазы не начаты. Фазы 2-4 ждут согласованного эскиза (фаза 0).
