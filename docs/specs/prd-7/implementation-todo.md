...
> **S9 закрыта 2026-05-27.** Канонический статус с отметками `[x]` и ссылками на
> конкретные тест-кейсы — в [s9-s11-in-progress.md §2.2 (component) и §2.3 (API/regression)](./s9-s11-in-progress.md).
> Чек-листы ниже оставлены как историческая декомпозиция задач. Единственный открытый
> пункт — это фича, а не тест: раздел «Архив» с восстановлением в `tests-list`.
...
- [x] **FR-20c** — ссылки-якоря из сводки ошибок к проблемным полям по `field`-путям
  `ValidationIssue`. Реализовано 2026-05-27 (Banner + `tabForField`/`goToError` +
  `data-field`-якоря); см. s9-s11-in-progress.md §2.2.
