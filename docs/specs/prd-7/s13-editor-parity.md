...
| G17 | partial **2026-05-28** | author-row row-menu теперь содержит «Сменить вариант…» (когда `variants.length > 1` для данного kind) перед «Удалить» (`data-testid="structure-page-replace-${id}"`); переиспользует существующий `ReplaceVariantModal` через `handlers.onReplaceVariant`. **Остаток**: пункт «Предпросмотр» (FR-44) — page-preview modal не реализован, требует scope-решения (iframe runtime vs lightweight stub). Выделено в **G17b/S13.4b** | (state `s-row-menu-open`) |
