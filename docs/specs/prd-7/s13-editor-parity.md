...
| G19 | partial | Structure-tab read-only визуал реализован (2026-05-28): `StructureSection` принимает `readOnly` (true при `test.status==="published"`), пробрасывается через `ZoneHandlers`; @dnd-kit отключён через `disabled` на page и topic-sortable; `.page-row--readonly` / `.topic-block--readonly` CSS-модификаторы дeglim grip; insert-rows скрыты; `PageEditForm` отключает inputs через `<fieldset disabled>` и заменяет «Сохранить»/«Отмена» на «Закрыть»; меню `…` скрыто на author-row, «Сменить вариант» disabled на system-row. **Остаток**: footer Drawer'а «Закрыть» вместо Save/Cancel — drawer-level, отдельный тикет | (state `s-readonly`, linear-by-topics, wf 790-852) |
