...
| ID | Severity | Что | Где |
| --- | --- | --- | --- |
| G1 | ~~critical~~ **closed 2026-05-28** | `tb-saving-overlay` рендерится внутри `.ou-drawer__body.tb-saving-host` при `combinedSaving`; inner content в `<div inert>` (нативный inert атрибут); spinner-svg + текст «Сохранение…». CSS spin keyframe добавлен | (state `s-saving`) |
| G2 | ~~substantial~~ **closed 2026-05-28** | `useTestEditor` экспортирует `snapshot` (последний сохранённый model); `ChangesPopover` вычисляет per-field diff `label · old → new` через `collectChangesByTab` для 9 top-level полей (title/description/mode/flowMode/runtime.*/webhookUrl/telemetry); группы по табам, счётчик `1 изменение / 2-4 изменения / 5+ изменений`; fallback «Структурные изменения» для tab без primitive-diff'ов | (state `s-changes`, wf 871-918) |
...
| G5 | ~~substantial~~ **closed 2026-05-28** | `ConflictDiffTable` подкомпонент в `ConflictDialog`: один-shot GET `/api/tests/:id` при открытии, парсит через `apiToEditorModel`, рендерит `tb-conflict-diff` table с тремя колонками (Поле / На сервере / Ваши изменения); фильтрует только те 8 top-level полей, где значения отличаются; loading/error state. ConflictDialog переведён на size=l чтобы fit table | (state `s-conflict`) |
...
| ID | Severity | Что | Где |
| --- | --- | --- | --- |
| G7 | ~~critical~~ **closed 2026-05-28** | Реализован `TestFeedbackTrigger` + `Card variant="outlined"` секция «Общая обратная связь теста» в Основное pane: открывает `FeedbackEditorModal` с rich-text body, links list, PDF assets (полные fields, без `hideAssets`); сохраняет format/text/links/assets в `model.basic.feedback*`. Preview-кнопка показывает snippet + meta «N ссылок · M PDF» | (state `s-basic`, wf 710-839) |
| G8 | ~~critical~~ **closed 2026-05-28** | Switch «Показывать правильные ответы после прохождения» перенесён из LimitsPane в BasicPane → внутри карточки «Общая обратная связь теста» после `<hr>` (per wireframe 820-837); `data-testid="settings-show-correct-checkbox"` сохранён. Старый тест в Limits-describe удалён, новый добавлен в Основное-describe | — |
...
| G16 | ~~substantial~~ **closed 2026-05-28** | Поиск `variant-search__input` добавлен в `AddPageModal` — переиспользует ту же визуальную структуру и тестовые id-конвенции, что и `ReplaceVariantModal` (S13.6-G28). Скрыт когда вариантов ≤3 (мало смысла фильтровать), empty-state «Ничего не найдено». State `s-page-preview` (страница-предпросмотр) закрыт в S13.4-G17 через `PagePreviewModal` (отдельная entry для row-menu «Предпросмотр»; add-modal не требует preview, поскольку выбирается ещё не созданная страница) | (states `s-add-step1`, `s-page-preview`) |
...
| ID | Severity | Что | Где |
| --- | --- | --- | --- |
| G28 | ~~critical~~ **closed 2026-05-28** | `ReplaceVariantModal` получил `variant-search` поиск (case-insensitive фильтр по `label`+`description`), empty-state «Ничего не найдено», `is-current` модификатор + «Текущий» chip на текущем варианте, и `diff-block` warning «Часть настроек не переносится» со списком теряемых placeholder values (рендер `label: «value»`, HTML-теги стрипаются, длинные строки обрезаются до 60 chars). Контракт diff: `placeholder.key` сравнивается между current/new variant (PRD-1 §4.3.3) | (state `s-replace-modal`, wf 132-213) |
| G29 | ~~substantial~~ **closed 2026-05-28** | Когда `selectedVariant.placeholders.length === 0` AND variant отличается от текущего: diff-block использует другой заголовок «Текущие настройки страницы будут потеряны» + meta «У нового варианта нет редактируемых полей — содержимое страницы будет полностью задано шаблоном» | (state `s-replace-no-fields`, wf 263-317) |

### 1.5 Close-confirm (`prd7-editor-close-confirm.html`)

| ID | Severity | Что | Где |
| --- | --- | --- | --- |
| G30 | ~~substantial~~ **closed 2026-05-28** | `CloseConfirmDialog` рендерит `.tb-close-confirm-chips` снизу modal-body — Tag-список (variant=outline tone=neutral) затронутых dirty-вкладок из `editor.tabStatuses`; пер-tab `data-testid` для регрессий | (state `s-dirty`, wf 134-147) |
| G31 | ~~substantial~~ **closed 2026-05-28** | При `hasErrors && errorFieldCount > 0` поверх chips рендерится inline `Banner tone="error"` со счётчиком («N ошибк[а/и/ок] во вкладке «X»») + action «Перейти к первой ошибке» — вызывает `goToError(firstErrorField)` (закрывает confirm и фокусирует поле). Plural form склонения подсчёта на ru | (state `s-errors`, wf 184-194) |
