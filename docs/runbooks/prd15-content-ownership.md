...
**Канонический механизм выкладки:** прогон нумерованных SQL-файлов через `scripts/db/run-sql.cjs`
(`node scripts/db/run-sql.cjs <file.sql> [<file.sql> ...]`, читает `DATABASE_URL`, exit ≠ 0 при
...
СТАТУС НА СЕГОДНЯ: деплой переведён на `drizzle-kit migrate` (версионированные миграции в
`drizzle/`), а два скрипта выкладки объединены в один `scripts/deploy/deploy.sh` — он одинаков для
прода и тестового инстанса и применяет миграции в одноразовом контейнере до старта приложения.
`push --force` в деплое не используется, `run-sql.cjs` остался инструментом ad-hoc правок.
Описание ниже относится к прежней push-эпохе и сохранено как исторический контекст.

Прежние деплой-скрипты `deploy.sh` и `deploy-test.sh` это делали так: в pre-push шаге гоняли
цепочку `016, 023, 024, 026, 027, 028` (deploy-test роли бэкфиллит инлайном) через `run-sql.cjs`
ДО `drizzle-kit push --force`, поэтому push выходил no-op (создание `test_question_scoring` и дропы
...
Файлы `migrations/` применяются последовательно по номеру через `scripts/db/run-sql.cjs` (все шаги
