...
экрана (`start.*` со свойством страницы `image`).
...
### 7.3a Отчёт о результатах: `report.*` (PRD-27)
...
`settings[]` — те же поля, что у контентных страниц (см. §7.3 PRD-22), их значения автор
теста заполняет в блоке обратной связи раздела «Настройки». Ограничения отчёта: тип
...
| `attemptsCountLabel` | Готовая подпись числа попыток, со склонением |
...
| `backgroundUrl` / `logoUrl` / `hasLogo` | Подложка и логотип, уже разрешённые в строку |
| `isPreview` | Предпросмотр в настройках теста: можно показать пометку «образец» |
| `values.<key>` | Значения `settings[]` варианта; поля `image` приходят строкой-URL |

В адаптивном отчёте строки `result.topicResults[]` дополнительно несут `hasCounts`,
`answeredLabel` и `correctLabel` — готовые подписи числа заданных и верных вопросов.
...
файлы из рантайма поздно. Пакеты, собранные до PRD-27, этого блока не несут и продолжают
собирать отчёт по каноническому виду.
