...
**Статус:** актуально; соответствует спецификации формата версии 1.0.0 и эталонному шаблону
«Стандартный». Учтены системные узлы раздела (`review`, `section-results`) и варианты стартового
экрана (`start.*` + параметр `startImageUrl`).

**Версия руководства:** 1.0.0 · **соответствует спецификации формата:** 1.0.0 ·
...
- [Платформа SCORM-шаблонов](../specs/spec-template-platform.md) — формальная
  спецификация формата (источник истины), версия 1.0.0.
...
```json
{
  ...
  "params": [
    ...
    },
    {
      "key": "startImageUrl",
      "type": "image",
      "label": "Изображение старта",
      "default": null,
      "group": "Оформление",
      "section": "branding",
      "description": "Иллюстрация в правой колонке стартового экрана."
    }
  ],
  "contentTemplates": [
    {
...
Иллюстрацию задаёт параметр оформления `startImageUrl` (`design.startImageUrl`, §7.5), общий для
всех вариантов старта, — отдельного placeholder у стартовой страницы нет.
...
`design.*` (везде, где показывается брендинг): `logoUrl` — URL логотипа, `startImageUrl` —
URL иллюстрации стартового экрана (оба уже разрешены в строку из media-конверта параметра).
...
(с плейсхолдером, пока изображение не задано). Значения приходят из параметров оформления
`logoUrl` / `startImageUrl` (тип `image`, секция `branding`).
...
Помимо цветов, секция `branding` несёт два media-параметра (тип `image`): `logoUrl` — логотип
в шапке старта и итогов, `startImageUrl` — иллюстрация правой колонки стартового экрана
(`design.logoUrl` / `design.startImageUrl`, §7.5). Оба необязательны (`default: null`).
