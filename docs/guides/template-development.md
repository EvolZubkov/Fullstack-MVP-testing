...
**Статус:** актуально; соответствует спецификации формата версии 1.1.0 и эталонному шаблону
«Стандартный». Учтены системные узлы раздела (`review`, `section-results`), варианты стартового
экрана (`start.*` со свойством страницы `image`) и страница отчёта о результатах
(`report` / `report.adaptive`, §7.3a).

**Версия руководства:** 1.1.0 · **соответствует спецификации формата:** 1.1.0 ·
...
- [Платформа SCORM-шаблонов](../specs/spec-template-platform.md) — формальная
  спецификация формата (источник истины), версия 1.1.0.
...
`settings[]` — те же поля, что у контентных страниц (§8.1), их значения автор
