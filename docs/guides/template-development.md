...
Правило одно, и его легко нарушить: **если у экрана есть вариант в `contentTemplates[]`, макет
берётся из поля `layoutFile` этого варианта**, а не из ключа в `layouts[]`. Ключи `layouts[]`
нужны только системным экранам без варианта (`shell`, `content`, `question`, `results`,
`review`, `section-results`, `system.*`).
...
```json
{
  ...
  "placeholders": [{ "key": "instruction", "type": "richText", "label": "Инструкция" }]
}
```

Ключа вида `layouts["content.intro"]` в контракте нет: такой ключ ядро не читает, макет
окажется недостижим, и экран молча отрисуется из стандартного шаблона.

Placeholders варианта должны соответствовать макету: ядро вставляет области
`data-placeholder` по описанию варианта. Если вариант объявляет `title`/`subtitle`, а макет
ожидает `instruction`, значения автора отрисованы не будут.
