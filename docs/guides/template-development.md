...
**2. Галерея — по ключу своего варианта.** Слайд галереи (`kind: gallery`) ядро ищет по
`layouts[<ключ варианта>]` (значение поля `key` элемента `contentTemplates[]`).

**3. Прочие контентные страницы — по `layoutFile` варианта.** Для `info` / `summary` / `router` /
