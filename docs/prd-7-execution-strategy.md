...
Регрессия на момент закрытия S0-S8: ~19 файлов × ~457 тестов, `npm run check`
0 ошибок.

Незакрытый пункт S4/4A — FR-20c (anchor-навигация из summary ошибки к проблемному
полю) — перенесён в S9 как часть component-тестов.
...
| --- | --- | --- | --- | --- | --- |
| S9 | 6A | Component-тесты + FR-20c | Haiku | 7A | Не начата (следующая) |
| S9 | 6B | API и regression тесты | Haiku | 7A | Не начата |
