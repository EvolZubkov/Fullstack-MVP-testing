...
- [~] 12. Разбить `DatabaseStorage` на доменные репозитории за фасадом
  ...
  - Остаток фасада (вне исходных 10 доменов, полный сплит по согласованию):
    ...
    - [x] 12.13. Folders — `server/storage/folders-repository.ts`
      (`FoldersRepository`): два организационных дерева — папки контента
      `folders` (держат темы) и папки тестов `test_folders` (держат тесты).
      Варианты удаления работают одной транзакцией: «только папка» переносит
      темы/тесты и дочерние папки в назначение (по умолчанию корень),
      `deleteTestFolderCascade` удаляет все тесты поддерева вместе с папками.
      Папка — корень агрегата операции, поэтому `topics`/`tests` пишутся прямо в
