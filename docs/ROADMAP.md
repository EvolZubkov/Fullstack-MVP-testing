...
**Последняя актуализация:** 2026-06-12 (Трек PRD-15 «Владение контентом,
...
[implementation-plan.md](./specs/prd-15/implementation-plan.md) — **фазы 0-3
(блоки A, B, C) реализованы 2026-06-11/12**. Блок A — целостность: `created_by`
...
**Открыты: T-40 (удаление колонок вопроса — отдельный релиз) и фаза 5 закрытия
(T-41..T-44)**. Ранее
...
| 9 | **[PRD-15 (Владение контентом, целостность и публикация)](./specs/prd-15/content-ownership.md)** — фазы 0-4 реализованы 2026-06-11/13, миграции 019-027; блок D (оценка — свойство теста) готов целиком: `test_question_scoring` + умолчания (026), резолвер во всех потребителях, backfill (027), лист «Оценка» книги Excel, клиентская вкладка «Оценка» с переносом конструктора градуировки (эскиз prd15-test-scoring в approved/); ОТКРЫТЫ T-40 (удаление колонок вопроса — отдельный релиз) и фаза 5 закрытия (приёмка E-1..E-13, матрица, RUNBOOK) | Целостность (защита зависимых тестов + dry-run), публикация через снапшоты (неизменность опубликованного, доигрывание попыток на своей версии, экстренная переопубликация), владение темами (видимость, гранты user-only TD-01, двухрежимный отзыв, одноимённость), богатая обратная связь темы `feedback_json` (TD-02), оценка как свойство теста (переопределения, умолчания, эффективная сложность в адаптиве) | PRD-13 (модель прав), PRD-14 (книга Excel), PRD-10 (цена ответа) |
