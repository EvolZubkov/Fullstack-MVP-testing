...
**Последняя актуализация:** 2026-06-04 (PRD-10 «Цена ответа» + PRD-11 «Квоты выдачи по тегам»
ЗАКРЫТЫ — оба RTK-трека реализованы end-to-end и запушены; см. §0.2. Ранее: 2026-05-29 post-MVP
...
| 9 | PRD-12 | Единый шаблонный рантайм рендера (L2): один DSL-рендерер + публичный контекст для SCORM и веб; серверный расчёт `@shared`; поглощает платформу PRD-3 | Не начата — план v0.2 (2026-06-05), ожидает go на Фазу 0 | PRD-2/4/5/6/10/11; spec-template-platform |
...
| — | **[PRD-12 (Единый шаблонный рантайм рендера)](./specs/prd-12/web-runtime-parity.md)** — план v0.2, охват L2 | Один DSL-рендерер + публичный контекст для SCORM и веб; серверный расчёт `@shared`; убирает хардкод-рендер в обеих средах; поглощает платформенную часть PRD-3 | PRD-2/4/5/6/10/11; spec-template-platform |
