...
**Версия:** 1.5
**Статус:** Утверждено
**Источник:** [BRD](./specs/brd-scorm-enhancements.md), PRD-1...PRD-11
...
постобработки», см. §0.2 и §3.4; PRD-4 + PRD-8 закрыты — Storyline-MVP shippable. 2026-06-03:
PRD-2 + PRD-5 закрыты; открыт трек «Цена ответа» (PRD-10) + «Квоты выдачи по тегам» (PRD-11) —
scoring-model §11 + BRD BR-09/BR-10, перед релизом РТК-сертификации, см. §0.2)
...
| 2 | **[PRD-10 (Цена ответа)](./specs/prd-10/graded-answer-scoring.md)** | Градуированный балл: веса опций (`single`) + ступенчатая таблица «условие→балл» (`multiple`/`matching`/`ranking`); `scoreRatio`, статус «Частично правильно». Интернализует 2-й внешний постпроцессор (РТК). План: Стадия 0a | BR-09; перед релизом РТК |
| 3 | **[PRD-11 (Квоты выдачи по тегам)](./specs/prd-11/tag-draw-quotas.md)** | Опц. квота выдачи `[тег→N]` на секцию (стратификация по подтемам); гарантирует объём подтемы. Доставка, ортогональна скорингу. План: Стадия 0a | BR-10; перед релизом РТК |
| 4 | PRD-6 | Retake gate / cooldown (compliance для корпоративных курсов) | PRD-4 |
| 5 | PRD-3 | Реестр и жизненный цикл внешних шаблонов | PRD-1 |
...
**Трек «Цена ответа» (PRD-10) + «Квоты выдачи» (PRD-11) — открыт 2026-06-03.** Нормативная модель —
[scoring-model.md](./specs/scoring-model.md) §11 (v1.9: «Цена ответа» — веса опций / ступенчатая таблица);
планы — [PRD-10](./specs/prd-10/graded-answer-scoring.md) (балл за ответ) и
[PRD-11](./specs/prd-11/tag-draw-quotas.md) (квоты выдачи по тегам/подтемам). Драйвер — РТК-сертификация
(идёт на платформу сразу после релиза). Выверено по ТЗ-2026 (`key_NEW_15-08-25.xlsx` + pandas):
`single`-веса — авторинг, `multiple`/`matching` — ступенчатая оценка (кодовый пробел, #17); подтемы
(Тема = тег) — квоты выдачи (PRD-11). Эскиз редактора «Цены ответа» готов и выверен (Playwright), на
согласовании; редактор квот — Стадия 0b.
