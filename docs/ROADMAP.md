...
**Версия:** 1.13
**Статус:** Утверждено
**Источник:** [BRD](./specs/brd-scorm-enhancements.md), PRD-1...PRD-20
**Последняя актуализация:** 2026-07-02 (Релиз **2.7.0-beta**. Закрыт трек **PRD-20
«Устойчивость таймера и возобновление timed-тестов»** — BR-14/Этап 15, спека
[PRD-20](./specs/prd-20/timer-integrity-resume.md), план
[PLAN_prd20_timer_phase2.md](./PLAN_prd20_timer_phase2.md); **Фазы 1-2 реализованы и
локально верифицированы 2026-07-02** (2932 теста + Playwright), LMS-валидация Фазы 2 —
техдолг перед включением в прод. Таймер экзамена переведён на монотонный `performance.now()`:
устранён дрейф в фоновой вкладке и выигрыш от перевода системных часов в пределах сессии;
секционный таймер PRD-4 — на тот же примитив. Кросс-reload целостность на модели активного
времени: baseline-anchor `{ limitMinutes, baselineTotalSec, sig }` в `suspend_data.timer` +
`cmi.total_time` — timed-тесты снова резюмятся (снят технический guard `saveCurrentSession`;
adaptive остаётся исключением — переигрывание/последний результат); периодический
`Commit(session_time)` раз в 10 с даёт kill-устойчивость; tamper-evidence (keyed
djb2-checksum на anchor, дезтеррент) превращает правку `suspend_data` в потерю resume, а не
«фри тайм»; watchdog отката часов снят в пользу флага аномалии при регрессе `total_time`,
счётчики reload/сессий отложены. Единообразное возобновление по режимам BR-06: линейные — с
точного вопроса (в т.ч. внутри темы), router — внутри незавершённой темы (позиция
`currentRouterTopic` + `currentPageIndex`, реконструкция чанка). Любой сбой персистентности
деградирует к прежнему
поведению без регресса, в т.ч. тесты без лимита.) Ранее 2026-07-01 (Релиз **2.6.0-beta**.
После закрытия PRD-19 (2026-06-29)
закоммичены: **PRD-19 системные узлы границ раздела** — введение/обзор/итоги раздела как системные
узлы шаблона (`intro` per-topic, `review`/`section-results` singletons), убраны legacy `intro`/`summary`;
чинит пустую страницу «Далее» в начале раздела и дублирующиеся «Итоги раздела»; предпросмотр страниц
«Структуры» из реальных данных; миграции 034/035 идемпотентны. **Переработка конструктора показателей
PRD-2** — карточка в 3 блока (Имя·Метка / Формула / Вывод), 4 шаблона (Порог/Категория/Взвешенная
сумма/Вердикт) на едином условии-примитиве (Элемент·Свойство·Оператор·Значение), читаемые ссылки на
темы `topicById(код)`/`topicByName(имя)` без UUID, общий балл `score`, справочник функций со вставкой
по курсору, опциональная метка показателя, код темы `topics.code` (миграция 032), хинт о
нереализованном источнике «Другие шкалы». **PRD-16 групповые операции «Папки и темы»** — два
взаимоисключающих режима выделения, групповой перенос/доступ/видимость/владелец, двухрежимное удаление
папки, partial-batch с защитой контента (серверные bulk-эндпоинты `topics/bulk-*` и
`folders/bulk-reparent` плюс клиент); difficulty стал nullable (миграция 029). Прочее: открытие
редактора теста двойным кликом,
кеш-политика SPA (no-cache на index.html), импорт книги PRD-14 с владельцем и оценкой из легаси-колонок,
фильтр контента по владельцу. Ранее 2026-06-29 (Трек PRD-19 «Навигация прохождения» — спека
...
## 0. Текущий статус (на 2026-07-02)
