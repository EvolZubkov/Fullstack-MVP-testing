...
PRD-2 + PRD-5 закрыты, открыт трек PRD-10 «Градуированная оценка ответа» (нормативная модель
scoring-model §11, план PRD-10), поднят перед релизом РТК-сертификации, см. §0.2)
...
| 2 | **[PRD-10 (Градуированная оценка ответа)](./specs/prd-10/graded-answer-scoring.md)** | Тировая рубрика балла для `multiple`/`matching`/`ranking` + веса опций для `single`; `scoreRatio`, статус «Частично правильно». Интернализует 2-й внешний постпроцессор (РТК-сертификация). План: Стадия 0a | BR-09; перед релизом РТК |
| 3 | PRD-6 | Retake gate / cooldown (compliance для корпоративных курсов) | PRD-4 |
...
**Трек PRD-10 (Градуированная оценка ответа) — открыт 2026-06-03.** Нормативная модель —
[scoring-model.md](./specs/scoring-model.md) §11 (v1.9, тировые рубрики); план реализации —
[PRD-10](./specs/prd-10/graded-answer-scoring.md) (Стадия 0a). Поднят как ближайший пост-MVP
драйвер: РТК-сертификация руководителей (второй внешний Excel-постпроцессор сверх MBI) идёт на
платформу сразу после релиза и без градуированной оценки не реализуется. Выверено по данным
ТЗ-2026 (`key_NEW_15-08-25.xlsx` + pandas-обработчик): веса опций `single` — авторинг (сетка
вкладов), `multiple`/`matching` — ступенчатая рубрика (реальный кодовый пробел, issue #17).
Дальше — Стадия 0b (эскизы редактора рубрик), на согласовании.
