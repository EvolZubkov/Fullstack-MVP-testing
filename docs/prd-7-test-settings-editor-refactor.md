...
| FR-29 | Вкладка "Структура" должна учитывать режим прохождения: для последовательного режима показывать темы, для router-режима показывать страницу-маршрутизатор и карточки разделов, для смешанного режима показывать единый блок вопросов и зоны "Перед тестом" / "После теста" |
...
| FR-33 | Редактор должен иметь явный параметр `flowMode`, определяющий сценарий прохождения: `linear_flat`, `linear_by_topics`, `mixed` или `router_by_topics` |
...
| FR-40 | При смене `flowMode` структура пересобирается из default-структуры нового режима; несовместимые пользовательские элементы скрываются с warning, но не удаляются |
...
type TestEditorModel = {
  ...
  flowMode: "linear_flat" | "linear_by_topics" | "mixed" | "router_by_topics";
  flowSettings: {
...
- в смешанном режиме, где все вопросы перемешиваются, структура не показывает темы как блоки
  прохождения;
- для смешанного режима структура показывает зоны **"Перед тестом"**, **"Блок вопросов"** и
  **"После теста"**.
