...
**Версия:** 1.2  
**Статус:** Утверждено для реализации (S0 + S1 закрыты 2026-05-21: контракты
зафиксированы в [decisions.md](prd-7-decisions.md), wireframes 153/153
согласованы)  
**Последняя актуализация:** 2026-05-22 (FR/NFR приведены в соответствие
с утверждёнными wireframes: mobile out-of-scope в FR-43, FR-36/37/37a расширены
под `s-feedback-edit`, добавлены FR-46 variant-replace и FR-47 под-вкладки
«Настройки», FR-25f/25g переформулированы под FR-40 info-banner модель)  
...
Layout-требования (desktop ≥ 960px; mobile < 960px вынесен за scope PRD-7,
покрывается отдельным PRD):

- desktop width: `min(1120px, calc(100vw - 48px))`;
- вкладка "Настройки" использует side nav второго уровня и правую рабочую область;
...
| FR-25f | При смене `flowMode` редактор показывает **info-banner** «Режим изменён на X. Содержимое Состава и Структуры сохранено» — без warning, без callout «несовместимые элементы будут преобразованы». Системные параметры режима (например, router-page) сохраняются в draft до закрытия редактора и не визуализируются в `linear_*` (см. FR-40, [decisions.md §2.3b](prd-7-decisions.md)) |
| FR-25g | Редактор не предоставляет отдельное действие ручной очистки скрытых системных параметров режима; при возврате к router-режиму в той же сессии параметры маршрутизатора восстанавливаются автоматически (FR-40) |
...
| FR-36 | Обратная связь поддерживается на уровнях теста, темы и adaptive-уровня; содержание feedback редактируется не во вкладке "Структура". Контент feedback хранится в формате `format: 'plain' \| 'richText' \| 'html'`; формат выбирается inline через SegmentedControl в модале редактирования feedback (state `s-feedback-edit` editor-drawer). Для `richText` редактор поддерживает базовое форматирование (bold, italic, link) через toolbar; `html` принимает сырой HTML с серверной санитизацией |
| FR-37 | Feedback может содержать PDF-документы (assets), которые упаковываются в SCORM и выводятся через download links на страницах структуры. Asset поддерживает inline-rename имени документа в модале feedback-редактора; mime-type фиксирован `application/pdf` |
| FR-37a | Feedback может содержать «Ссылки на материалы» — список внешних ссылок с парой `title` + `url`; хранятся в `feedback_json.links[]` (см. [decisions.md §3.4](prd-7-decisions.md)). Редактируются в той же модали `s-feedback-edit` отдельной секцией от PDF-assets |
...
| FR-43 | Редактор теста использует wide Drawer; вкладка "Настройки" имеет второй уровень навигации (side nav). Mobile (< 960px) вынесен за scope PRD-7 и покрывается отдельным PRD |
...
| FR-46 | Смена варианта существующей страницы (`page-row`) выполняется через `…` row-menu → «Сменить вариант» с подтверждением через модал `prd7-variant-replace.html` (см. [PRD-1 §4.3](prd-1-templates-content-pages.md)); модал показывает diff потерь параметров. Backend поддерживает endpoint `POST /api/tests/:id/pages/:pageId/replace-variant`, который возвращает diff и применяет смену с правилом «имя поля = контракт между вариантами одного `kind`» |
| FR-47 | Вкладка «Настройки» содержит фиксированный набор под-вкладок: «Основное», «Правила прохождения», «Ограничения», «Интеграция», «Адаптивный режим». Feedback теста редактируется в «Основное» (а не отдельной под-вкладкой); `flowMode` редактируется в «Правила прохождения» (а не отдельной «Режим и сценарий»); под-вкладка «Адаптивный режим» видна только при `mode = adaptive` |
