...
**Решение по месту вызова и обработке сбоя** (вопрос поднят при исполнении Задачи 6).

Синхронизация вызывается ПОСЛЕ фиксации сущности, отдельным шагом, НЕ в одной транзакции с
сохранением, и её сбой НЕ роняет сохранение — он только пишется в журнал.

Обоснование от последствий. Если завернуть индекс в транзакцию с контентом, сбой при разрешении
одной легаси-ссылки откатит правку вопроса целиком: автор потеряет работу из-за неполадки,
не имеющей к его правке отношения. Если же сбой переживается, худшее следствие — недостающая
строка индекса, а она даёт ОТКАЗ в доступе, а не выдачу лишнего, и лечится пересборкой из
Задачи 11, которая ровно для этого и существует.

Поэтому оборачивай каждый вызов так:
