...
describe("buildTestJson — предел показа шкалы (PRD-46 §6)", () => {
  ...
  it("шкала без предела несёт ключ со значением «не задано»", () => {
    // Интерпретация уезжает в пакет ЦЕЛИКОМ, поэтому ключ на месте всегда, а «не задано»
    // выражено значением. Пропуск ключа сэкономил бы байты и вернул бы ровно тот класс
    // ошибки, из-за которого предел показа не доехал в пакет: правило «что-то возим, а
    // что-то нет» приходится помнить, а `null` не помнит никто.
    const out = parse(buildTestJson(exportData(DOMAIN)));
    expect(out.scales[0].displayMax).toBeNull();
  });
