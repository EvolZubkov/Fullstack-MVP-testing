 ...
 * разобрал», смещение «Номер» ↔ `level_index` на единицу, порядок уровней внутри темы,
 * умолчание пустого названия и построчные ошибки — занятый номер, неизвестный тип порога,
 * границы сложности.
 */
...
describe("лист «Адаптивные уровни»", () => {
  ...
  // Редактор имя уровня не проверяет — уровень с пустым именем сохраняется штатно, и
  // выгрузка пишет пустую ячейку. Отвергать её значило бы сделать книгу строже редактора
  // (PRD-48 §9), а у единственного уровня темы отказ строки отменил бы весь `save`.
  it("пустое «Название» получает умолчание редактора «Уровень N»", () => {
    const { errors, byTopic } = parseAdaptiveLevelSheet([
      row({ "Номер": 1, "Название": "  " }),
      row({ "Номер": 2, "Название": "" }),
      row({ "Номер": 3, "Название": "Продвинутый" }),
    ]);
    expect(errors).toEqual([]);
    expect(byTopic.get("финансы")?.map((l) => l.levelName)).toEqual([
      "Уровень 1", "Уровень 2", "Продвинутый",
    ]);
  });
