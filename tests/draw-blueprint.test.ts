 ...
 * (PRD-11 §5; FR-02/03/03a/03b/04/06).
 */
...
describe("drawSection — uniform (no blueprint, FR-02)", () => {
  ...
  it("an empty-strata blueprint also falls back to uniform", () => {
    const r = drawSection(QS, 2, { modeGranularity: "uniform", mode: "exact", strata: [] }, identity);
    expect(ids(r.selected)).toEqual(["1", "2"]);
  });
});

describe("drawSection — exact strata", () => {
  const bp: DrawBlueprint = { modeGranularity: "uniform", mode: "exact", strata: [{ tag: "A", count: 2 }] };
  ...
  it("a multi-tag question counts once (FR-04)", () => {
    ...
    const two: DrawBlueprint = {
      modeGranularity: "uniform",
      mode: "exact",
      strata: [{ tag: "A", count: 1 }, { tag: "B", count: 1 }],
    };
  ...
  it("a shortfall is a non-blocking warning (FR-06)", () => {
    const big: DrawBlueprint = { modeGranularity: "uniform", mode: "exact", strata: [{ tag: "A", count: 5 }] };
...
describe("drawSection — min strata (FR-03a)", () => {
  it("takes count as a floor; remainder may include the same tag", () => {
    ...
    const bp: DrawBlueprint = { modeGranularity: "uniform", mode: "min", strata: [{ tag: "A", count: 2 }] };
