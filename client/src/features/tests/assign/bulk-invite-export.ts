...
export interface LinksWorkbookOptions {
  ...
  /**
   * Срок, до которого действуют ссылки, — тот, что вернул прогон (ISO), а не
   * тот, что оператор ввёл на форме: при пустом поле настоящий срок известен
   * только серверу (срок сдачи, иначе +30 дней). `null` — прогон не выпустил ни
   * одной ссылки, и печатать в колонке нечего.
   */
