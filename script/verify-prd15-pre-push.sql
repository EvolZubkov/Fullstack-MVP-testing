-- PRD-15 pre-push safety gate. Run via `node script/run-sql.cjs` AFTER the
...
BEGIN
  IF to_regclass('public.test_question_scoring') IS NULL THEN
    RAISE EXCEPTION
      'PRD-15 gate: table test_question_scoring missing -> migration 026 did not run before push. Run migrations 023,024,026,027,028 via script/run-sql.cjs, then retry. Aborting to avoid data loss.';
  END IF;

  IF to_regclass('public.topic_courses') IS NOT NULL
     OR to_regclass('public.topic_events') IS NOT NULL THEN
    RAISE EXCEPTION
      'PRD-15 gate: topic_courses/topic_events still present -> migrations 023 (backfill) + 024 (drop) did not run before push. Run them via script/run-sql.cjs, then retry. Aborting to avoid losing recommendations.';
  END IF;

  IF EXISTS (SELECT 1 FROM information_schema.columns
             WHERE table_name = 'questions' AND column_name IN ('points', 'scoring_json')) THEN
    RAISE EXCEPTION
      'PRD-15 gate: questions.points/scoring_json still present -> migrations 027 (backfill) + 028 (re-backfill+drop) did not run before push. Run them via script/run-sql.cjs, then retry. Aborting to avoid losing custom scoring.';
  END IF;
