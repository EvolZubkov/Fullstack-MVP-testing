...
/**
 * Section-local ids of the template-authoring documents, mapped to the shared
 * registry (`server/services/doc-downloads`). The short `guide`/`spec` URLs are
 * kept: the buttons in the «Шаблоны» toolbar link to them.
 */
const SECTION_DOCS: Record<string, string> = {
  guide: "template-development",
  spec: "template-spec",
};

/**
 * GET /api/admin/templates/docs/:doc — download a template-authoring document
 * (`guide` = руководство разработчика, `spec` = спецификация платформы) as PDF.
 * Registered before `/:id` so `docs` is never read as a template id. Returns 404
 * with a hint when the artifact is missing (regenerate with `npm run docs:pdf`).
 */
router.get("/docs/:doc", requirePermission("adminTemplates.manage"), async (req, res) => {
  try {
    ...
    logger.error("Doc download error: " + (error as Error).message);
