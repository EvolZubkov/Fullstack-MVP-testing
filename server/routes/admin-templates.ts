...
/**
 * Template-authoring documents offered for download in the «Шаблоны» section,
 * keyed by the URL id. PDFs are pre-generated by `npm run docs:pdf` into
 * `docs/dist`, so the server ships them without needing Chrome at runtime.
 */
const DOC_DOWNLOADS: Record<string, { file: string; filename: string }> = {
  guide: { file: "template-development.pdf", filename: "Руководство по разработке шаблонов.pdf" },
  spec: { file: "spec-template-platform.pdf", filename: "Спецификация платформы шаблонов.pdf" },
};
