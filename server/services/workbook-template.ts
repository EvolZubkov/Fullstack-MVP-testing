...
const HELP_ROWS: HelpRow[] = [
  ...
  [SHEET_QUESTIONS, "Тип вопроса", "да", "Одно из: multiple_choice (одиночный выбор) | multiple_response (множественный) | matching (сопоставление) | ranking (ранжирование) | scale (шкала). От типа зависит формат двух следующих колонок.", "multiple_choice"],
  ...
  [SHEET_QUESTIONS, "Тексты вариантов ответа", "да", "Варианты через «#». Для matching два списка через «||»: «лево # лево || право # право» — правый может быть длиннее, лишние станут ловушками. Для ranking — элементы в произвольном порядке. Для scale — градации шкалы по порядку, от одного полюса к другому.", "Прибыль # Денежный поток # Активы"],
  [SHEET_QUESTIONS, "Номера правильных ответов", "да", "Счёт с 1. multiple_choice: один номер «2». multiple_response: несколько «1,3». matching: пары «лево-право» через запятую «1-1, 2-2» (пусто = по порядку). ranking: правильный порядок номеров «2,1,3», каждый ровно один раз. scale: ОДИН номер либо ПУСТО — пустая ячейка означает опросник без правильного ответа.", "1,2"],
  ...
  [SHEET_STRUCTURE, "Случайный порядок вопросов", "нет", "да (по умолчанию) — вопросы раздела перемешиваются, как раньше. нет — выдаются по колонке «Индекс в теме» листа «Вопросы»; в режиме вариантов — в порядке списка варианта. В тесте со сплошным списком вопросов настройка не действует: там перемешиваются вопросы всех разделов.", "нет"],
