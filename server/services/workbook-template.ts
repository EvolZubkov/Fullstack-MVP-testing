...
const VALIDATED_COLUMNS: Record<string, Record<string, string[]>> = {
  ...
  // называет свои темы. «Уровень» же — закрытая пара, а опечатка в ней уводит строку
  // не к тому владельцу или отвергает её целиком.
  [SHEET_FEEDBACK]: {
    "Уровень": LEVEL_CHOICES,
...
export const EXAMPLE_ROWS: Record<string, Record<string, unknown>[]> = {
  ...
  [SHEET_STRUCTURE]: [
    { "Раздел": "Финансы", "Порядок": 1, "Вопросов в выборке": 3, "Тип порога": "Сумма баллов", "Порог": 4, "Обязательный": "да", "Случайный порядок вопросов": "да" },
  ...
  [SHEET_RECOMMENDATIONS]: [
    { "Уровень": "Тест", "Раздел": "", "Тип": "Курс", "Заголовок": "Финансы для руководителя", "Ссылка": "https://example.test/finance" },
    { "Уровень": "Тест", "Раздел": "", "Тип": "Материал", "Заголовок": "Памятка по отчётности", "Ссылка": "https://example.test/memo.pdf" },
    // У мероприятия ссылки может не быть вовсе: очная встреча живёт в расписании,
    // а не на странице записи.
    { "Уровень": "Тест", "Раздел": "", "Тип": "Мероприятие", "Заголовок": "Вебинар «Разбор отчётности»", "Ссылка": "" },
    { "Уровень": "Раздел", "Раздел": "Финансы", "Тип": "Курс", "Заголовок": "Основы финансового учёта", "Ссылка": "https://example.test/accounting" },
  ],
...
const HELP_ROWS: HelpRow[] = [
  ...
  [SHEET_FEEDBACK, "Уровень", "да", "Чья это обратная связь: Тест (общая, на экране итогов) | Раздел (по одной теме).", "Раздел"],
  [SHEET_FEEDBACK, "Раздел", "да, для уровня «Раздел»", "Название темы — как на листе «Структура». Для уровня «Тест» ячейка должна быть ПУСТОЙ: заполненная — ошибка строки, а не молчаливо отброшенное значение.", "Финансы"],
  ...
  ["", "", "", "Уровень и раздел разведены двумя колонками намеренно: тема с названием «Тест» — обычное дело, и в одной общей колонке такая строка молча ушла бы на уровень всего теста.", ""],
  ...
  [SHEET_RECOMMENDATIONS, "Уровень", "да", "Чьей обратной связи принадлежит рекомендация: Тест | Раздел. Читается так же, как на листе «Обратная связь».", "Тест"],
  [SHEET_RECOMMENDATIONS, "Раздел", "да, для уровня «Раздел»", "Название темы. Для уровня «Тест» ячейка пустая. Владелец обязан быть назван на листе «Обратная связь», иначе строка отвергается: рекомендация хранится внутри его обратной связи.", "Финансы"],
  [SHEET_RECOMMENDATIONS, "Тип", "да", "Курс | Материал | Мероприятие. От типа зависит только строгость колонки «Ссылка».", "Курс"],
  ...
  ["", "", "", "Вопросы → Шкалы → Показатели → Вклады вопросов → Оценка → Структура + Квоты + Пороги вариантов + Обратная связь + Рекомендации → Страницы + Поля страниц. Поэтому ссылки на вопросы, созданные этим же файлом, работают.", ""],
...
function buildExampleLines(): ExampleLine[] {
  const out: ExampleLine[] = [
    ...
    { kind: "note", cells: ["раздел с двумя вариантами и своими порогами, шкала с диапазонами, два показателя,"] },
