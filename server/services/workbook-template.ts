...
export const EXAMPLE_ROWS: Record<string, Record<string, unknown>[]> = {
  ...
  [SHEET_STRUCTURE]: [
    { "Раздел": "Финансы", "Порядок": 1, "Вопросов в выборке": 3, "Тип порога": "Сумма баллов", "Порог": 4, "Обязательный": "да" },
    { "Раздел": "Право", "Порядок": 2, "Вопросов в выборке": 1, "Тип порога": "Процент", "Порог": 50, "Обязательный": "да" },
...
const HELP_ROWS: HelpRow[] = [
  ...
  ["", "", "", "Пустая ячейка = «не задано»: подставится значение по умолчанию. Это не ошибка.", ""],
  ...
  [SHEET_QUESTIONS, "Тип вопроса", "да", "Одно из: multiple_choice (одиночный выбор) | multiple_response (множественный) | matching (сопоставление) | ranking (ранжирование). От типа зависит формат двух следующих колонок.", "multiple_choice"],
  ...
  [SHEET_QUESTIONS, "Тексты вариантов ответа", "да", "Варианты через «#». Для matching два списка через «||»: «лево # лево || право # право» — правый может быть длиннее, лишние станут ловушками. Для ranking — элементы в произвольном порядке.", "Прибыль # Денежный поток # Активы"],
  [SHEET_QUESTIONS, "Номера правильных ответов", "да", "Счёт с 1. multiple_choice: один номер «2». multiple_response: несколько «1,3». matching: пары «лево-право» через запятую «1-1, 2-2» (пусто = по порядку). ranking: правильный порядок номеров «2,1,3», каждый ровно один раз.", "1,2"],
  [SHEET_QUESTIONS, "Следование вариантов ответов", "нет", "Fixed — не перемешивать варианты. Любое другое значение или пусто — перемешивать (по умолчанию). Fixed нужен, когда есть «все перечисленное» или варианты образуют шкалу.", "Fixed"],
  ...
  ["", "", "", "Тест с этим листом переключается в режим со страницей-маршрутизатором: ученик видит список разделов и выбирает, с какого начать.", ""],
  GAP,
  ...
  ["", "", "", "«веса» — ТОЛЬКО для multiple_choice: свой балл за каждый вариант, по порядку вариантов. Недостающие веса считаются нулями, лишние — ошибка. Максимум за вопрос = наибольший вес.", "веса: 2 # 1 # 0"],
  ...
  ["", "", "", "«ступени» — ТОЛЬКО для multiple_response, matching, ranking: лестница условий. Ступени через «;», условия внутри ступени через «&», операторы == >= <= < >. Проверяются сверху вниз, побеждает первая подошедшая, иначе 0.", "ступени: correct == total & wrong == 0 => 2; correct >= 1 => 1"],
  ...
  [SHEET_SCALES, "Диапазоны", "нет", "Именованные интервалы: «мин..макс код «подпись»», через «;». Границы включаются. Подпись необязательна.", "0..49 low «Базовый»; 50..100 high «Экспертный»"],
  [SHEET_SCALES, "Показывать ученику", "нет", "да | нет (по умолчанию).", "да"],
  ...
  [SHEET_MEASUREMENTS, "Источник", "да", "За что начисляется: вопрос (за верный ответ целиком) | вариант | пара (для matching) | позиция (для ranking).", "вариант"],
  [SHEET_MEASUREMENTS, "Ключ источника", "нет", "ВНИМАНИЕ: счёт с НУЛЯ, в отличие от «Номеров правильных ответов». Для источника «вопрос» — пусто. Для «вариант» — номер варианта: 0, 1, 2. Для «пара» и «позиция» — два числа через двоеточие.", "0"],
  ...
  [SHEET_RESULT_VARS, "Показывать ученику", "нет", "да | нет (по умолчанию).", "да"],
  ...
  ["", "", "", "Вопросы → Шкалы → Показатели → Вклады вопросов → Оценка → Структура + Квоты + Пороги вариантов. Поэтому ссылки на вопросы, созданные этим же файлом, работают.", ""],
